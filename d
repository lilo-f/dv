...
if (!isset($_SESSION['user_id'])) {
    // Se você quer permitir algumas ações sem login, ajuste conforme necessário
    $allowedActionsWithoutLogin = ['get_favorites']; // Exemplo
    if (!in_array($input['action'] ?? '', $allowedActionsWithoutLogin)) {
...
case 'add_to_cart':
    ...
    $userId = $_SESSION['user_id']; // Já validamos que existe
    error_log("Recebida requisição add_to_cart"); // Log de depuração
    error_log("Dados recebidos: " . print_r($input, true)); // Log dos dados recebidos

    if (!isset($_SESSION['user_id'])) {
        error_log("Usuário não logado tentou adicionar ao carrinho");
    ...
    error_log("Tentando adicionar produto ID: $productId, quantidade: $quantity para usuário ID: $userId");

    if (empty($productId) || !is_numeric($productId) || $quantity <= 0) {
        error_log("Dados inválidos recebidos: product_id=$productId, quantity=$quantity");
...
case 'get_cart_items':
  ...
  case 'add_to_favorites':
    if (empty($userId)) {
        throw new Exception("Você precisa estar logado para adicionar favoritos.");
    }
    ...
    // Verifica se o produto já está nos favoritos para evitar duplicatas
    ...
    $response = ['success' => true, 'message' => 'Produto adicionado aos favoritos com sucesso!'];
    break;
...
case 'remove_from_favorites':
    ...
    $response = ['success' => true, 'message' => 'Produto removido dos favoritos com sucesso!'];
    break;
...
case 'get_favorites':
        ...
        ");
        $stmt->execute([$userId]);
        $favorites = $stmt->fetchAll(PDO::FETCH_COLUMN, 0); // Pega apenas a coluna 'product_id'
